...
func (_q *TeamQuery) QueryOtpInvites() *OtpInvitesQuery {
...
func (_q *TeamQuery) WithOtpInvites(opts ...func(*OtpInvitesQuery)) *TeamQuery {
	query := (&OtpInvitesClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	...
	if query := _q.withOtpInvites; query != nil {
		if err := _q.loadOtpInvites(ctx, query, nodes,
			func(n *Team) { n.Edges.OtpInvites = []*OtpInvites{} },
			func(n *Team, e *OtpInvites) { n.Edges.OtpInvites = append(n.Edges.OtpInvites, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TeamQuery) loadOtpInvites(ctx context.Context, query *OtpInvitesQuery, nodes []*Team, init func(*Team), assign func(*Team, *OtpInvites)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Team)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(otpinvites.FieldTeamID)
	}
	query.Where(predicate.OtpInvites(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(team.OtpInvitesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
