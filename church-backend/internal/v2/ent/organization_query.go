...
func (_q *OrganizationQuery) QueryBranches() *BranchQuery {
...
func (_q *OrganizationQuery) QueryPeople() *PersonQuery {
...
func (_q *OrganizationQuery) WithBranches(opts ...func(*BranchQuery)) *OrganizationQuery {
	query := (&BranchClient{config: _q.config}).Query()
...
func (_q *OrganizationQuery) WithPeople(opts ...func(*PersonQuery)) *OrganizationQuery {
	query := (&PersonClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(organization.FieldVersion).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Organization.Query().
//		Select(organization.FieldVersion).
//		Scan(ctx, &v)
...
func (_q *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := _q.withBranches; query != nil {
		if err := _q.loadBranches(ctx, query, nodes,
			func(n *Organization) { n.Edges.Branches = []*Branch{} },
			func(n *Organization, e *Branch) { n.Edges.Branches = append(n.Edges.Branches, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *OrganizationQuery) loadBranches(ctx context.Context, query *BranchQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Branch)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(branch.FieldOrganizationID)
	}
	query.Where(predicate.Branch(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.BranchesColumn), fks...))
	}))
...
func (_q *OrganizationQuery) loadPeople(ctx context.Context, query *PersonQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Person)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(person.FieldOrganizationID)
	}
