...
type DelegationQuery struct {
	...
	withParentGrant *ScopedGrantQuery
...
// QueryParentGrant chains the current query on the "parent_grant" edge.
func (_q *DelegationQuery) QueryParentGrant() *ScopedGrantQuery {
...
func (_q *DelegationQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *DelegationQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (_q *DelegationQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *DelegationQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (_q *DelegationQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (_q *DelegationQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *DelegationQuery) Clone() *DelegationQuery {
	...
	return &DelegationQuery{
		...
		withParentGrant: _q.withParentGrant.Clone(),
...
// WithParentGrant tells the query-builder to eager-load the nodes that are connected to
// the "parent_grant" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *DelegationQuery) WithParentGrant(opts ...func(*ScopedGrantQuery)) *DelegationQuery {
	query := (&ScopedGrantClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withParentGrant = query
	return _q
}
...
//		GroupBy(delegation.FieldVersion).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Delegation.Query().
//		Select(delegation.FieldVersion).
//		Scan(ctx, &v)
...
func (_q *DelegationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Delegation, error) {
	...
	if query := _q.withParentGrant; query != nil {
		if err := _q.loadParentGrant(ctx, query, nodes, nil,
			func(n *Delegation, e *ScopedGrant) { n.Edges.ParentGrant = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *DelegationQuery) loadParentGrant(ctx context.Context, query *ScopedGrantQuery, nodes []*Delegation, init func(*Delegation), assign func(*Delegation, *ScopedGrant)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Delegation)
	for i := range nodes {
		fk := nodes[i].ParentGrantID
	...
	query.Where(scopedgrant.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "parent_grant_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *DelegationQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(delegation.Table, delegation.Columns, sqlgraph.NewFieldSpec(delegation.FieldID, field.TypeUUID))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withParentGrant != nil {
			_spec.Node.AddColumnOnce(delegation.FieldParentGrantID)
		}
	}
